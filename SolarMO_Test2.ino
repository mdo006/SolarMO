...
long turnTick45 = 58; //turn 45 degrees
long turnTick90 = 116; //turn 90 degrees
long turnTick180 = 232; //u-turn
...
void turnRight45() 
{
  ...
  while(rightTick - prevTickR <= turnTick45 || leftTick - prevTickL <= turnTick45) {
    rightTurn();
  }
  halt();
}
...
void turnRight90() 
{
  ...
  while(rightTick - prevTickR <= turnTick90 || leftTick - prevTickL <= turnTick90) {
    rightTurn();
  }
  halt();
}
...
void uTurn() 
{
  ...
  while(rightTick - prevTickR <= turnTick180 || leftTick - prevTickL <= turnTick180) {
    leftTurn();
  }
  halt();
}
